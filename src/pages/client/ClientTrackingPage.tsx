...
import { clientSubmittedRows, type ClientProposalRow } from "./clientPengajuanData";
...
function getProposalStatusLabel(row: ClientProposalRow) {
  if (row.status === "Selesai") return "Selesai";
  if (row.status === "Proses") return row.progressLabel ?? "Proses";
  return "Ditolak";
}

function getProposalStatusVariant(status: ClientProposalRow["status"]) {
...
export function ClientTrackingPage() {
  ...
  const selectedProposal = useMemo<ClientProposalRow>(() => {
    return clientSubmittedRows.find((row) => row.pengajuan === search?.pengajuan) ?? clientSubmittedRows[0];
  }, [search?.pengajuan]);
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
              <div className="min-w-0">
                ...
                <p className="mt-2 max-w-3xl text-[14px] leading-7 text-white/85">
                  Halaman client memusatkan riwayat proses dan respon instansi agar status pengajuan mudah ditelusuri tanpa aksi edit.
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Badge variant={getProposalStatusVariant(selectedProposal.status)}>{getProposalStatusLabel(selectedProposal)}</Badge>
              </div>
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-3">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/95 px-4 py-4 text-left text-neutral-900 shadow-sm">
                ...
                <p className="mt-2 break-words text-[14px] font-semibold leading-6 text-neutral-800">{getProposalStatusLabel(selectedProposal)}</p>
              </div>
            </div>
          </div>
        </CardBody>
